...
CodeFire provides persistent memory across Claude Code sessions via MCP. Tasks, notes, and project context survive between conversations.
...
Use for understanding how features work, finding patterns, and exploring code BEFORE making changes. Prefer over Grep/Glob when searching for concepts rather than exact strings.
...
Prefer these over running `git` via Bash:
